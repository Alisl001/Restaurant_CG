...
// restaurant floor
	...
	glRotatef(rightDoorAngle, 0.0f, 1.0f, 0.0f); // Rotate around y axis
		glTranslatef(-141.0, 150.0, -567.0);
	glTranslatef(141.0, -150.0, 567.0);
	...
	glPushMatrix();
	glTranslatef(-141.0, -150.0, 567.0);
	glRotatef(leftDoorAngle, 0.0f, 1.0f, 0.0f); // Rotate around y axis
		glTranslatef(141.0, 150.0, -567.0);
	glTranslatef(-141.0, -150.0, 567.0);
	glScalef(0.5, 1.0, 1.0);
...
int InitGL(GLvoid)										// All Setup For OpenGL Goes Here
{
  ...
  MyCamera.Position.z = +1550;
	...
	//tvModel = new Model_3DS();
	//tvModel->Load("TV.3ds");
	//Match.LoadBMP("match.bmp");
	//tvModel->Materials[0].tex=Match;
	//tvModel->pos.x=0;
	//tvModel->pos.y=0;
	//tvModel->pos.z=0;
	//tvModel->scale=1;
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	// map floor
	glPushMatrix();
	glTranslatef(0.0, 7690.0, 0.0);
	glBindTexture(GL_TEXTURE_2D, grassTex);
	glBegin(GL_QUADS);
        // Bottom face
		//glColor4f(0.3f, 0.5f, 0.1f, 0.0f);
        glTexCoord2f(0.0f, 0.0f); glVertex3f(-8500, -8500, -8500);//-x, -y, -z
        glTexCoord2f(10.0f, 0.0f); glVertex3f(8500,  -8500, -8500);//x, -y, -z
        glTexCoord2f(10.0f, 10.0f); glVertex3f(8500,  -8500,  8500);//x, -y, z
        glTexCoord2f(0.0f, 10.0f); glVertex3f(-8500, -8500,  8500);//-x, -y, z
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Ali Sliman's Restaurant",640,480,16,fullscreen))
	{
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			if (keys['P'])						// Is P Being Pressed?
			{
				while (leftDoorAngle < 90) {
					leftDoorAngle += 3;}
				while (rightDoorAngle > -90) {
					rightDoorAngle -= 3;}
			}
			if (keys['O'])						// Is O Being Pressed?
			{
				while (leftDoorAngle > 0) {
					leftDoorAngle -= 3;}
				while (rightDoorAngle < 0) {
					rightDoorAngle += 3;}			
			}
			if (keys[VK_F1])						// Is F1 Being Pressed?
			{
				...
				if (!CreateGLWindow("Ali Sliman's Restaurant",1920,1080,1024,fullscreen))
				{
					return 0;						// Quit If Window Was Not Created
				}
			}
